...
#PBS -l gpunum_job=2
#PBS -o ../gmlp_logs/gmlp_denoise_same_model_lambda500.out
#PBS -e ../gmlp_logs/gmlp_denoise_same_model_lambda500.err
#PBS -r n
...
echo "Running script: scripts/gmlp_denoise_same_model.py"
...
BATCH="${BATCH:-256}"
LR="${LR:-1e-3}"
INPUT="${INPUT:-/sqfs/work/cm9029/${USER_ID}/gmlp_project/data/denoise/mnist_lambda500.npz}"
OUTDIR="${OUTDIR:-/sqfs/work/cm9029/${USER_ID}/gmlp_output/denoise_same_model/lambda500}"
...
CUDA_VISIBLE_DEVICES="$STR_GPUS_TRAIN" python scripts/gmlp_denoise_same_model.py \
    --epochs "$EPOCHS" \
    --patch "$PATCH" \
    --d_model "$DMODEL" \
    --d_ffn "$DFFN" \
...
CUDA_VISIBLE_DEVICES="$STR_GPUS_VAL" python scripts/gmlp_denoise_same_model.py \
    --epochs "$EPOCHS" \
    --patch "$PATCH" \
    --d_model "$DMODEL" \
    --d_ffn "$DFFN" \
...
if [ $RC_TRAIN -eq 0 ] && [ $RC_VAL -eq 0 ]; then
    echo "✅ gMLP denoise same-model job completed successfully (Both Train & Val)"
    exit_code=0
else
    echo "❌ gMLP denoise same-model job failed"
