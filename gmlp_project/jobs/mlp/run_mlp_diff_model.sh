...
#PBS -l gpunum_job=4
#PBS -o ../../logs/mlp_diff_model.out
#PBS -e ../../logs/mlp_diff_model.err
#PBS -r n

#------- Program execution -----------

echo "🚀 Starting MLP dual-model job (diff)"
echo "======================================"
...
OUTDIR="${OUTDIR:-/sqfs/work/cm9029/${USER_ID}/mlp_output/diff_model}"
EXTRA_ARGS="${EXTRA_ARGS:-}"

# === 作業ディレクトリ (SSD) 設定 ===
REPO_ROOT="/sqfs/work/cm9029/${USER_ID}/gmlp_project"
SCRATCH_BASE="/sqfs/ssd/cm9029/${USER_ID}"
JOB_LABEL="mlp_diff_${PBS_JOBID:-manual_$$}"
...
CUDA_VISIBLE_DEVICES=0 python "$REPO_ROOT/scripts/mlp_diff_model.py" \
    --epochs "$EPOCHS" \
    --d_model "$DMODEL" \
    --d_ffn "$DFFN" \
    --num_blocks "$BLOCKS" \
...
CUDA_VISIBLE_DEVICES=1 python "$REPO_ROOT/scripts/mlp_diff_model.py" \
    --epochs "$EPOCHS" \
    --d_model "$DMODEL" \
    --d_ffn "$DFFN" \
    --num_blocks "$BLOCKS" \
...
if [ $EXIT_TRAIN -eq 0 ] && [ $EXIT_VAL -eq 0 ]; then
    echo "✅ MLP diff-model job completed successfully (Both Train & Val)"
    exit_code=0
else
    echo "❌ MLP diff-model job failed (Train=$EXIT_TRAIN, Val=$EXIT_VAL)"
    echo "--- Train Log tail ---"
