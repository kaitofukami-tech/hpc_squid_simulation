...
"""
gMLP (with SGU) dual-model denoising experiment recorder
- Same-mode: A/B は完全に同一初期状態（A→Bへ state コピー）で開始し、独立シャッフルで学習
- スピン観測: gMLP 各ブロックの TokenBN (:pre/:post) と out_bn
- 記録エポック: {1,3}+プログレッシブ。MSE/PSNR を同じタイミングで Train/Val を評価して保存
"""
...
# ----------------- argparse -----------------
def build_args():
    p = argparse.ArgumentParser("gMLP+SGU dual-model denoising experiment (same init)")
    p.add_argument(
        "--input",
        type=str,
        default="/sqfs/work/cm9029/${USER_ID}/gmlp_project/data/denoise/mnist_lambda1.npz",
        help="Path to npz generated by build_denoise_datasets.py",
    )
    p.add_argument("--output_dir", type=str, default="/sqfs/work/cm9029/${USER_ID}/gmlp_output/denoise_same_model")
...
def load_denoise_npz(path, logger):
    npz_path = Path(path)
    with np.load(npz_path) as f:
        required = ["noisy_train", "clean_train", "noisy_test", "clean_test"]
        for key in required:
            if key not in f:
                raise KeyError(f"{npz_path} missing '{key}'")
        noisy_train = f["noisy_train"]
        clean_train = f["clean_train"]
        noisy_test = f["noisy_test"]
        clean_test = f["clean_test"]
        lambda_value = float(f["lambda_value"]) if "lambda_value" in f else float("nan")
    ...
    data = {
        "noisy_train": prep(noisy_train),
        "clean_train": prep(clean_train),
        "noisy_test": prep(noisy_test),
        "clean_test": prep(clean_test),
        "labels_train": torch.from_numpy(labels_train) if labels_train is not None else None,
        "labels_test": torch.from_numpy(labels_test) if labels_test is not None else None,
        "lambda_value": lambda_value,
        "source": str(npz_path),
    }
    logger.info(
        f"Loaded denoise set from {npz_path.name}: "
        f"train={tuple(data['noisy_train'].shape)}, test={tuple(data['noisy_test'].shape)}, lambda={lambda_value}"
    )
    return data
...
def save_metrics_csv(path, rows, logger):
    ...
    with open(path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["epoch","split","loss","psnr"])
        writer.writeheader()
...
class gMLP(nn.Module):
    ...
    def forward(self, x_img):  # (B,1,28,28)
        ...
        x = self.reconstruct(x)                                                  # (B,S,p^2)
        grid = self.image_size // self.patch
        x = x.view(B, grid, grid, self.patch, self.patch)
        x = x.permute(0, 1, 3, 2, 4).contiguous().view(B, 1, self.image_size, self.image_size)
...
# ----------------- train / eval -----------------
def mse_to_psnr(mse: float, max_val: float = 1.0, eps: float = 1e-8) -> float:
    mse = max(mse, eps)
...
def train_one_epoch(model, loader, opt, loss_fn, logger, tag):
    ...
    for noisy, clean in loader:
        noisy = noisy.to(d, non_blocking=True)
        clean = clean.to(d, non_blocking=True)
        opt.zero_grad(set_to_none=True)
        recon = model(noisy)
        loss = loss_fn(recon, clean)
        loss.backward(); opt.step()
        total += float(loss) * noisy.size(0); n += noisy.size(0)
        del noisy, clean, recon, loss
        if torch.cuda.is_available(): torch.cuda.empty_cache()
    mse = total / max(n, 1)
    psnr = mse_to_psnr(mse)
    logger.info(f"[{tag}] train_mse={mse:.5f} psnr={psnr:.2f}dB")
    return mse, psnr
...
def main():
    ...
    run_name = f"run_gmlp_denoise_same_{ts}_p{args.patch}_{args.measure_data}"
    ...
    # data
    data = load_denoise_npz(args.input, logger)
    noisy = data["noisy_train"]
    clean = data["clean_train"]
    ...
    train_ds, val_ds = random_split(dataset, [n_train, n_val], generator=gsplit)
    ...
    if args.measure_data == "train":
        ...
        source_data = dataset.tensors[0][source_indices]
        logger.info(f"Measure Target: TRAIN (from split, N={len(source_data)})")
    else:
        # val_ds は TensorDataset(noisy_val, clean_val) なので [0] が noisy
        # ただし val_ds は Subset ではなく TensorDataset or Subset なので注意
        # ここでは random_split しているので val_ds は Subset
        # Subset の場合、indices を使って元データから取る必要がある
        # しかし make_loader で dataset を渡しているので
        # ここでは元の noisy_test (load_denoise_npz で読んだもの) を使うのが安全だが、
        # load_denoise_npz は train/test しか返さない。
        # random_split 後の val_ds からデータを取り出す
        # val_ds[i] -> (noisy, clean)
        # これを全量取り出すのは重いかもしれないが、M個だけならインデックス操作で済む
        # ここではシンプルに "noisy_test" (load_denoise_npzの返り値) を使うか、
        # あるいは val_ds から取り出すか。
        # 既存コードでは train_ds, val_ds は random_split で作られている
        # build_denoise_datasets.py は noisy_test を作っているが、
        # main 内では train_ds, val_ds が split されている。
        # 実は load_denoise_npz は "train" と "test" を返しているが、
        # main では "dataset" (train + test ? いや、load_denoise_npz の実装を見るに train だけ dataset にしている)
        # L420: dataset = TensorDataset(noisy, clean) -> noisy=noisy_train
        # つまり random_split は train データセットを分割している。
        # args.input の noisy_test は使われていない？
        # L108-109 で noisy_test 読み込んでいるが、
        # L418-420 で noisy_train だけ使って dataset を作っている。
        # つまり val_ds は train データの一部。
        # 一方、mlp_denoise_same_model.py は load_denoise_data で train_ds, val_ds を返している。
        # gmlp のこのスクリプトは "train.npz" 内の train を分割して val にしているようだ。
        # なので val_ds から取るのが正しい。
        
        # Subset からの取り出し
        val_indices = val_ds.indices
        source_data = dataset.tensors[0][val_indices] # noisy
        logger.info(f"Measure Target: VAL (from split, N={len(source_data)})")

    idx_meas = rng.choice(len(source_data), size=min(args.M, len(source_data)), replace=False)
    X_meas = source_data[torch.from_numpy(idx_meas)].clone()
    ...
    modelA = gMLP(28, args.patch, args.d_model, args.d_ffn,
              args.num_blocks, 1, args.dropout).to(d)
    ...
    metrics_rows_A = []  # dict(epoch, split, loss, psnr)
    metrics_rows_B = []
    ckpt_records_A = []
    ckpt_records_B = []
    ...
    for ep in range(1, args.epochs+1):
        ...
        if ep in rec_epochs:
            ...
            # ---- metrics (train/val) ----
            tr_loss_A, tr_psnr_A = eval_full(modelA, train_eval_loader, loss_fn)
            va_loss_A, va_psnr_A = eval_full(modelA, val_loader,         loss_fn)
            metrics_rows_A += [
                {"epoch": ep, "split": "train", "loss": f"{tr_loss_A:.6f}", "psnr": f"{tr_psnr_A:.4f}"},
                {"epoch": ep, "split": "val",   "loss": f"{va_loss_A:.6f}", "psnr": f"{va_psnr_A:.4f}"},
            ]
            tr_loss_B, tr_psnr_B = eval_full(modelB, train_eval_loader, loss_fn)
            va_loss_B, va_psnr_B = eval_full(modelB, val_loader,         loss_fn)
            metrics_rows_B += [
                {"epoch": ep, "split": "train", "loss": f"{tr_loss_B:.6f}", "psnr": f"{tr_psnr_B:.4f}"},
                {"epoch": ep, "split": "val",   "loss": f"{va_loss_B:.6f}", "psnr": f"{va_psnr_B:.4f}"},
            ]
            logger.info(f"[A/ep{ep}] eval train: mse={tr_loss_A:.4f} psnr={tr_psnr_A:.2f}dB | val: mse={va_loss_A:.4f} psnr={va_psnr_A:.2f}dB")
            logger.info(f"[B/ep{ep}] eval train: mse={tr_loss_B:.4f} psnr={tr_psnr_B:.2f}dB | val: mse={va_loss_B:.4f} psnr={va_psnr_B:.2f}dB")
    ...
    # ====== 保存 ======
    meta_common = dict(
        data_name=Path(args.input).stem,
        noise_lambda=data["lambda_value"],
        source=data["source"],
        task="denoise",
    ...
    pathA = os.path.join(outdir, f"gmlp_same_spinA_{args.measure_data}_D{args.d_model}_F{args.d_ffn}_L{args.num_blocks}_M{args.M}_seed{args.init_seed}_trA{args.train_seedA}.pkl")
    ...
    pathB = os.path.join(outdir, f"gmlp_same_spinB_{args.measure_data}_D{args.d_model}_F{args.d_ffn}_L{args.num_blocks}_M{args.M}_seed{args.init_seed}_trB{args.train_seedB}.pkl")
