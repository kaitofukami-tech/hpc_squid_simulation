...
- Based on gmlp_diff_model.py, adapted for MLP architecture.
- A/B: Different init seeds, same training data & shuffle order.
- Spin observation: MLP blocks' BN (:pre/:post) and out_bn.
- Recording epochs: {1,3} + progressive. loss/acc also recorded.
"""
...
# ----------------- argparse -----------------
def build_args():
    p = argparse.ArgumentParser("MLP dual-model spin recorder (diff init, same training data)")
    p.add_argument("--input", type=str, default="/sqfs/work/cm9029/${USER_ID}/gmlp_project/data/fashion_mnist.npz",
                   help="Fashion-MNIST .npz")
    p.add_argument("--output_dir", type=str, default="/sqfs/work/cm9029/${USER_ID}/True_mlp_output_diff_lossacc")
    ...
    p.add_argument("--init_seedB", type=int, default=456, help="Init seed for B (independent of A)")
    ...
    p.add_argument("--val_seed", type=int, default=None, help="Validation split seed (train_seed if None)")
    
    # Legacy/Compatibility args (ignored but kept to avoid breaking if passed)
...
def setup_logger(outdir, level="INFO"):
    ...
    logger = logging.getLogger("MLP_Dual")
...
def load_npz_fashion(path, logger):
    ...
    # Flattening happens inside the model or here?
    # Keeping it as (N, 1, 28, 28) for consistency with loader logic, allowing model to flatten.
...
class MLPBlock(nn.Module):
    def __init__(self, d_model, d_ffn, dropout=0.1):
        super().__init__()
        # Pre-LN/BN structure usually puts Norm at start of residual branch?
        # gMLP was: x + Projection(SGU(Projection(BN(x))))
        # Here we do: x + Dropout(Projection2(GELU(Projection1(BN(x)))))
...
class MLP(nn.Module):
    ...
    def forward(self, x_img): # (B, 1, 28, 28)
        ...
        x = self.out_bn(x)
        # No mean pooling needed as we don't have sequence dim
...
def measure_spins(model: nn.Module, layer_names: List[str], X_meas: torch.Tensor, batch: int, logger):
    ...
    for k, v in store.items():
        store[k] = torch.cat(v, dim=0)   # (M, D) - no S dimension in MLP
...
def main():
    ...
    run_name = f"run_mlp_{ts}_{args.measure_data}"
    ...
    # Validation Split
    val_seed = args.val_seed if args.val_seed is not None else args.train_seed
    N = X.shape[0]
    ...
    # MLP specific layer names
    layer_list = [f"blocks.{i}.bn{args.record_layers}" for i in range(args.num_blocks)]
    # Optional: include record of out_bn? original gmlp logic commented it out or handled it differently?
    # Original: uncommented #layer_list += [f"out_bn{args.record_layers}"]
    # I will stick to what the original script had active (just blocks).
    ...
    # Dump A
    dumpA = {"meta": {**meta_common, "tag":"A", "init_seed":args.init_seedA,
                      "train_seed":args.train_seed, "time": timeA, "record_layers": layer_list,
    ...
    pathA = os.path.join(outdir, f"mlp_spinA_{args.measure_data}_D{args.d_model}_F{args.d_ffn}_L{args.num_blocks}_M{args.M}_seedA{args.init_seedA}.pkl")
    atomic_save_pickle(dumpA, pathA, logger)

    # Dump B
    dumpB = {"meta": {**meta_common, "tag":"B", "init_seed":args.init_seedB,
                      "train_seed":args.train_seed, "time": timeB, "record_layers": layer_list,
                      "metrics_epochs": rec_epochs, "checkpoints": ckpt_records_B}}
    for ln in layer_list:
        dumpB[ln] = np.stack([t.numpy() for t in spinsB[ln]], axis=0)
    pathB = os.path.join(outdir, f"mlp_spinB_{args.measure_data}_D{args.d_model}_F{args.d_ffn}_L{args.num_blocks}_M{args.M}_seedB{args.init_seedB}.pkl")
