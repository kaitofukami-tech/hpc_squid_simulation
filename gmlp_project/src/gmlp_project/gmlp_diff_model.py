...
- A/B: 異なる初期化シード、同一学習データ＆同一シャッフル順
...
# ----------------- argparse -----------------
def build_args():
    p = argparse.ArgumentParser("gMLP+SGU dual-model spin recorder (diff init, same training data)")
    ...
    p.add_argument("--output_dir", type=str, default="/sqfs/work/cm9029/${USER_ID}/gmlp_output/diff_model")
    ...
    # 学習/測定
    p.add_argument("--init_seedA", type=int, default=123, help="A の初期化シード")
    p.add_argument("--init_seedB", type=int, default=456, help="B の初期化シード（A と独立）")
    p.add_argument("--train_seed", type=int, default=2025, help="A/B 共通の学習シャッフル用シード")
    ...
    p.add_argument("--val_seed", type=int, default=None, help="検証分割用シード（未指定なら train_seed を使用）")
...
def setup_logger(outdir, level="INFO"):
    ...
    logger = logging.getLogger("gMLP_SGU_Dual")
...
def main():
    ...
    run_name = f"run_gmlp_{ts}_p{args.patch}_{args.measure_data}"
    ...
    pathA = os.path.join(outdir, f"gmlp_spinA_{args.measure_data}_D{args.d_model}_F{args.d_ffn}_L{args.num_blocks}_M{args.M}_seedA{args.init_seedA}.pkl")
    ...
    pathB = os.path.join(outdir, f"gmlp_spinB_{args.measure_data}_D{args.d_model}_F{args.d_ffn}_L{args.num_blocks}_M{args.M}_seedB{args.init_seedB}.pkl")
