...
#PBS -o ~/q2_dual_analysis_log_mlp_mlm_train_sup_ep1000.out
#PBS -e ~/q2_dual_analysis_log_mlp_mlm_train_sup_ep1000.err
#PBS -r n
...
echo "🚀 Starting MLP MLM dual q_inv figure job (train, epoch=1000)"
...
OUTDIR="${OUTDIR:-${REPO_ROOT}/thesis/mlp_qinv_out/mlp_mlm/train-val/sup_epoch1000}"
TITLE="${TITLE:-q_inv comparison (MLP MLM train-val, epoch=1000)}"
...
CMD=(python "${REPO_ROOT}/analysis_scripts/mlp_qinv_calc_fig_two_sets.py" \
  --spin_file_a1 ${MONO_ROOT}/mlp_output/mlp_mlm/diff_train/spin_A_Global.pkl\
  --spin_file_b1 ${MONO_ROOT}/mlp_output/mlp_mlm/diff_train/spin_B_Global.pkl\
  ${METRICS_A1:+--metrics_a1 "$METRICS_A1"} \
  ${METRICS_B1:+--metrics_b1 "$METRICS_B1"} \
  --label1       MLP_MLM_DIFF_train \
  --spin_file_a2  ${MONO_ROOT}/mlp_output/mlp_mlm/diff_val/spin_A_Global.pkl\
  --spin_file_b2  ${MONO_ROOT}/mlp_output/mlp_mlm/diff_val/spin_B_Global.pkl\
