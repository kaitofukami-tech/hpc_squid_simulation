...
OUTDIR="${OUTDIR:-${REPO_ROOT}/thesis/gmlp_mlm_qinv_out/10sentence_manifold_val}"
TITLE="${TITLE:-q_inv comparison (gMLP MLM 10-sentence val)}"
...
CMD=(python "${REPO_ROOT}/analysis_scripts/mlm_qinv_calc_fig_two_sets.py" \
  --spin_file_a1 /sqfs/work/cm9029/${USER_ID}/gmlp_output/mani/10manifold/diff_val/spin_A_Global.pkl\
  --spin_file_b1 /sqfs/work/cm9029/${USER_ID}/gmlp_output/mani/10manifold/diff_val/spin_B_Global.pkl\
  ${METRICS_A1:+--metrics_a1 "$METRICS_A1"} \
  ${METRICS_B1:+--metrics_b1 "$METRICS_B1"} \
  --label1       GMLP_MLM_DIFF_10sentence_val \
  --spin_file_a2  /sqfs/work/cm9029/${USER_ID}/gmlp_output/mani/10manifold/same_val/spin_A_Global.pkl\
  --spin_file_b2  /sqfs/work/cm9029/${USER_ID}/gmlp_output/mani/10manifold/same_val/spin_B_Global.pkl\
  ${METRICS_A2:+--metrics_a2 "$METRICS_A2"} \
  ${METRICS_B2:+--metrics_b2 "$METRICS_B2"} \
  --label2       GMLP_MLM_SAME_10sentence_val \
  "${LAYER_ARGS[@]}" \
  --title        "${TITLE}" \
  --output-dir   "${OUTDIR}")
