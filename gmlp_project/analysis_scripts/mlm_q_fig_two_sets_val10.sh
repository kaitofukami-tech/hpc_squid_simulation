...
CMD=(python "${REPO_ROOT}/analysis_scripts/mlm_qinv_calc_fig_two_sets.py" \
  --spin_file_a1 ${MONO_ROOT}/gmlp_output/mani/10manifold/diff_val/spin_A_Global.pkl\
  --spin_file_b1 ${MONO_ROOT}/gmlp_output/mani/10manifold/diff_val/spin_B_Global.pkl\
  ${METRICS_A1:+--metrics_a1 "$METRICS_A1"} \
  ${METRICS_B1:+--metrics_b1 "$METRICS_B1"} \
  --label1       GMLP_MLM_DIFF_10sentence_val \
  --spin_file_a2  ${MONO_ROOT}/gmlp_output/mani/10manifold/same_val/spin_A_Global.pkl\
  --spin_file_b2  ${MONO_ROOT}/gmlp_output/mani/10manifold/same_val/spin_B_Global.pkl\
