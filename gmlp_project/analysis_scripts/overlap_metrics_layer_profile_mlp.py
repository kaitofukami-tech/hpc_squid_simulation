...
"""
Layer-wise overlap metrics (intra/inter) for specified epochs (MLP version).
"""
...
def build_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Layer-wise overlap metrics for replica pairs (MLP)")
...
def resolve_layers(data: Dict[str, Any], user_layers: Optional[Sequence[int]]) -> List[str]:
    if user_layers:
        return [f"blocks.{i}.bn:post" for i in user_layers]
    pat = re.compile(r"blocks\.(\d+)\.bn:post")
    layers = [k for k in data.keys() if pat.fullmatch(k)]
    layers.sort(key=lambda k: int(pat.fullmatch(k).group(1)))
    if not layers:
        # Fallback default if regex fails for some reason
        layers = [f"blocks.{i}.bn:post" for i in range(10)]
    return layers
...
def compute_profiles(
    ...
    *,
    ...
    for layer in layers:
        # Expecting something like 'blocks.0.bn:post'
        ...
        for epoch in epochs:
            ...
            profiles["set1"].setdefault(epoch, {})[layer_idx] = metrics1["q_inv"]
            profiles["set2"].setdefault(epoch, {})[layer_idx] = metrics2["q_inv"]
            profiles["inter"].setdefault(epoch, {})[layer_idx] = metrics_inter["q_inv"]
    return profiles


def plot_profiles(
    profiles: Dict[str, Dict[int, Dict[int, float]]],
    epochs: Sequence[int],
    output: Path,
    *,
    title: Optional[str],
    ...
    for epi_idx, epoch in enumerate(epochs):
        ...
        for key in ["set1", "set2", "inter"]:
            ...
            if epoch_map is None:
                continue
            y = [epoch_map.get(idx, np.nan) for idx in layer_indices]
            if all(np.isnan(y)):
                continue
            ax.plot(
                layer_indices,
    ...
    ax.set_title(title or "Overlap metrics vs layer (MLP)")
    style_handles = [
