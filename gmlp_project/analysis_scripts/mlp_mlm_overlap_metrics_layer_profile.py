...
"""
Layer-wise overlap metrics (intra/inter) for specified epochs (MLP MLM).
"""
...
def build_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Layer-wise overlap metrics for replica pairs (MLP MLM)")
    ...
    p.add_argument("--output", default="./mlp_qinv_out/manifold/overlap_layer_profile.png")
...
def _layer_idx_from_key(key: str) -> int:
    m = re.search(r"(?:mlp_)?blocks\.(\d+)\.bn:post", key)
    if m:
        return int(m.group(1))
    return 0


def _layer_keys_from_data(data: Dict[str, Any]) -> List[str]:
    ...
    for k in keys:
        ...
        if re.fullmatch(r"(?:mlp_)?blocks\.\d+\.bn:post", str(k)):
            out.append(k)
    return sorted(out, key=_layer_idx_from_key)
...
def resolve_layers(data: Dict[str, Any], user_layers: Optional[Sequence[int]]) -> List[str]:
    ...
    if user_layers:
        prefix = "mlp_blocks" if any(k.startswith("mlp_blocks.") for k in keys) else "blocks"
        return [f"{prefix}.{i}.bn:post" for i in user_layers]
    if keys:
        return keys
    return [f"mlp_blocks.{i}.bn:post" for i in range(10)]
...
def intersect_layers(*layer_lists: List[str]) -> List[str]:
    ...
    for lst in layer_lists[1:]:
        shared &= set(lst)
    return sorted(shared, key=_layer_idx_from_key)
...
def extract_epoch(
    ...
    if _is_records_format(data):
        layers = data.get("_epoch_layers", {}).get(epoch, {})
...
def compute_profiles(
    ...
    *,
    ...
    def compute_epoch(epoch: int) -> Tuple[int, Dict[int, Dict[str, Any]]]:
        ...
        for layer in layers:
            layer_idx = _layer_idx_from_key(layer)
...
def plot_profiles(
    ...
    *,
    ...
    ax.set_title(title or "Inter-dataset q_inv vs layer (MLP)")
    color_handles = [
...
def main() -> int:
    ...
    if args.layers:
        ...
        if not layer_keys:
            raise ValueError("No blocks.*.bn:post layers found in all spin files.")
