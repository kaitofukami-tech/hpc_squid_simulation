...
"""
Recompute intermediate spins for saved gMLP checkpoints on a new dataset.

The script reloads the model hyper-parameters from the original spin
metadata (produced by gmlp_diff_model.py), iterates over the stored
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        "Recompute gMLP intermediate spins from saved checkpoints"
    )
    parser.add_argument(
        "--spin-pkl",
        required=True,
        help="Path to the original spin pickle (e.g. gmlp_spinA_*.pkl)",
    )
    parser.add_argument(
        "--dataset",
        required=True,
    ...
    parser.add_argument(
        ...
        default="train",
        choices=["train", "val"],
        help="Which split of the dataset to measure on (train or val). Default: train",
    )
    parser.add_argument(
        "--val-ratio",
        type=float,
        default=0.1,
        help="Validation ratio if splitting from train data (only used if dataset doesn't have split keys).",
    )
    return parser.parse_args()


def setup_logger(level: str, log_file: str | None = None) -> logging.Logger:
    logger = logging.getLogger("recompute_spins")
...
def _infer_model_config(state_dict: Mapping[str, torch.Tensor]) -> Dict[str, int]:
    """
    Infer gMLP architectural parameters from a checkpoint state dict.
    """
    try:
        patch_embed_weight = state_dict["patch_embed.weight"]
    except KeyError as exc:
        raise KeyError("Checkpoint missing 'patch_embed.weight' needed to infer patch size.") from exc
    patch_dim = patch_embed_weight.shape[1]
    patch_size = int(math.isqrt(patch_dim))
    if patch_size * patch_size != patch_dim:
        raise ValueError(
            f"Unable to infer square patch size from patch_embed.weight with second dimension {patch_dim}."
        )
    d_model = int(patch_embed_weight.shape[0])

    block_indices: List[int] = []
    for key in state_dict.keys():
        if key.startswith("gmlp_blocks."):
            parts = key.split(".")
            if len(parts) > 1 and parts[1].isdigit():
                block_indices.append(int(parts[1]))
    if not block_indices:
        raise ValueError("No gmlp_blocks.* entries found in checkpoint; cannot infer num_blocks.")
    first_block_idx = min(block_indices)
    num_blocks = max(block_indices) + 1

    channel_proj_key = f"gmlp_blocks.{first_block_idx}.channel_proj.weight"
    try:
        channel_proj_weight = state_dict[channel_proj_key]
    except KeyError as exc:
        raise KeyError(f"Checkpoint missing '{channel_proj_key}' needed to infer d_ffn.") from exc
    d_ffn = int(channel_proj_weight.shape[0])
...
def main() -> None:
    ...
    from gmlp_diff_model import (  # type: ignore
        gMLP,
        load_npz_fashion,
        measure_spins,
        atomic_save_pickle,
    ...
    logger.info(
        "Model configuration inferred from checkpoint: patch=%d d_model=%d d_ffn=%d num_blocks=%d num_classes=%d "
    ...
    # Use a custom loader logic here to handle both classification and denoising NPZs, 
    # and to handle train/val splitting logic consistent with training scripts.
    logger.info(f"Loading dataset from {dataset_path} for split: {args.measure_data}")
    
    # Try loading as simple classification first (x_train/y_train)
    # If that fails or if keys suggest denoising, handle accordingly.
    # Note: recompute_spins mainly needs 'X' (input).
    
    with np.load(dataset_path) as f:
        keys = set(f.keys())
    
    # Denoising keys?
    is_denoise = "noisy_train" in keys
    
    if is_denoise:
        # Denoising dataset logic
        from gmlp_denoise_diff_model import load_denoise_npz
        data_dict = load_denoise_npz(str(dataset_path), logger)
        # data_dict has "noisy_train", "noisy_test", etc. as Tensors
        
        # Determine source tensors based on measure_data
        # Note: Training scripts usually split "noisy_train" into train/val.
        # "noisy_test" is usually the separate test set. 
        # Here we follow the training script logic: split 'noisy_train'.
        
        full_train_inputs = data_dict["noisy_train"] # Tensor
        
        # Split logic
        N = len(full_train_inputs)
        # Use a fixed seed for splitting to match training if possible, 
        # though ideally we should use the same seed used during training.
        # The training script uses args.val_seed or args.train_seed.
        # Here we don't have the original train_seed easily accessible unless passed in.
        # We will use args.sample_seed as a proxy or 2025 default if not critical,
        # but to be precise, let's use a dedicated seed or just deterministic split.
        
        # For recompute, consistency is key. Let's use 2025 (default train_seed in scripts)
        # or allow user to pass it. simpler: just use args.sample_seed.
        split_seed = 2025 
        
        n_val = int(round(N * args.val_ratio))
        if n_val >= N: n_val = max(N - 1, 0)
        n_train = max(N - n_val, 1)
        
        gsplit = torch.Generator().manual_seed(split_seed)
        # We only need indices
        # We can't use random_split directly on Tensor easily without wrapping in Dataset, 
        # but we can just shuffle indices.
        
        indices = torch.randperm(N, generator=gsplit)
        train_indices = indices[:n_train]
        val_indices = indices[n_train:]
        
        if args.measure_data == "train":
            target_data = full_train_inputs[train_indices]
        else:
            target_data = full_train_inputs[val_indices]
            
        X_all = target_data
        
    else:
        # Standard classification logic
        # load_npz_fashion returns x_img, y_t (all train data usually)
        X_all_full, _ = load_npz_fashion(str(dataset_path), logger)
        ...
        X_all = X_all_full[target_indices]
