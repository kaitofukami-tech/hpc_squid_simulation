...
def build_args():
    ...
    p.add_argument("--q-block-size", type=int, default=0,
                   help="If >0, compute q2_* blockwise over feature dims to reduce memory (slower).")
...
def calc_qinv_epoch(A: np.ndarray, B: np.ndarray, *, use_nm: bool, combine_patches: bool,
    ...
    M, N = A2.shape
    if q_block_size and q_block_size > 0:
        q2_ab_raw = _q2_blockwise(A2, B2, q_block_size)
        q2_aa_raw = _q2_blockwise(A2, A2, q_block_size)
        q2_bb_raw = _q2_blockwise(B2, B2, q_block_size)
    else:
        q_ab = (A2.T @ B2) / M
        qaa = (A2.T @ A2) / M
        qbb = (B2.T @ B2) / M
