...
def build_args():
    p = argparse.ArgumentParser("q_inv over epochs for one A/B set on one figure")
    # Set 1
    ...
    p.add_argument("--all-layers", action="store_true", default=True,
                   help="Process and plot all layers (0..9)")
    p.add_argument("--title", type=str, default=None)
    p.add_argument("--output-dir", type=str, default="./gmlp_qinv_out/single")
    # Options
...
def setup_logger(level="INFO"):
    ...
    return logging.getLogger("qinv1")
...
def compute_qinv_df_for_layer(spinA_path: str, spinB_path: str, layer_key: str, *,
                              ...
                              patch_mean: bool, flatten: bool, logger: logging.Logger) -> pd.DataFrame:
...
def plot_single(fig_out: Path, layer_key: str, *,
                df: pd.DataFrame, label: str,
                skip_epoch0: bool, title: Optional[str], metrics_a: Optional[pd.DataFrame],
                metrics_b: Optional[pd.DataFrame]) -> None:
